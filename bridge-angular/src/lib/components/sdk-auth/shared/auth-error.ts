 ...
 * line; older ones (the published 0.7.0-beta.0) throw a plain `HttpError`
 * (status 403, that message), which is recognised here and logged here.
 *
 * Port of bridge-svelte's `sdk-auth/shared/auth-error.ts`.
 */
...
/** The translate function the components expose as `this.translate`. */
export type AuthErrorTranslate = Translator;
...
export function originNotAllowedMessage(
  t: AuthErrorTranslate,
  origin: string | undefined = pageOrigin(),
): string {
...
/**
 * The Error a component emits in place of `err` when it re-wraps it with a
 * display message (PasskeyLogin, SsoButton). An origin refusal stays
 * recognisable, so LoginForm can still show it (TBP-669).
 */
export function displayError(err: unknown, message: string): Error {
  const out = new Error(message);
  if (isOriginNotAllowed(err)) Object.assign(out, { code: 'ORIGIN_NOT_ALLOWED', status: 403 });
  return out;
}
 ...
 */
export function authErrorMessage(err: unknown, t: AuthErrorTranslate, fallbackKey: MessageKey): string {
