/**
 * `bridge.events.handle({...})` multiplex dispatcher — ported verbatim from
 * bridge-svelte (`core/events.ts`). Framework-agnostic pure TS; no Angular
 * primitives involved.
 *
 * One API for every channel-event kind. Discriminated union by `kind`:
 *
 *   const unsub = bridge.events.handle({
 *     'flag.updated':          (m) => log('flag', m.flag.key),
 ...
 * one throwing handler doesn't block the rest.
 *
 * The fallback `'*'` handler fires only when no specific handler is registered
 * for that kind.
 */
import type {
  FlagUpdateMessage,
 ...
 * key; `'*'` is the catch-all when no specific handler is registered.
 */
...
export class BridgeEventsDispatcher {
  ...
  /**
   * Internal — called by the runtime from every RealtimeClient hook. Routes by
   * `msg.kind` to registered handlers; falls back to `'*'` handlers only when no
   * specific handler exists for that kind. Never throws.
   */
