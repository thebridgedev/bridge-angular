/**
 * Feature Flags 2.0 — instance registry + change-notification bus.
 * Ported verbatim from bridge-svelte's `flags/registry.ts` (plain TS, no
 * framework primitives).
 *
 * The reactive layer (`flag-reactivity.ts`) subscribes here to surface
 * reactivity into Angular components/services via signals.
 */
...
/**
 * Emit a flag-change notification. `key === '*'` means "all flags may have
 * changed" (e.g. a bulk hydrate).
 */
...
/**
 * Non-reactive flag read. Use inside a `computed()` (after wiring a
 * subscription via the reactivity bridge) if you need reactivity from a
 * component context.
 *
 * Pass `context` to override the SDK's global EvalContext for this single call
 * — useful for rules that target dev-supplied attributes. Per-call attributes
 * win on key collision (auth-core locked decision #20).
 */
