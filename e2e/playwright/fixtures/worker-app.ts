 ...
 * Ported from bridge-svelte TBP-604 in TBP-721.
 *
 ...
 * that way gets deleted by the suite's own account purge. Worker 0 keeps the
 * owner this suite has always used for its unsuffixed app.
 */
export function workerAppOwnerEmail(parallelIndex: number): string {
  return parallelIndex === 0
    ? 'playwright-e2e@thebridge.io'
    : `iman+pw-angular-app-w${parallelIndex}@nebulr.group`;
}
 ...
 * specs that flip these flags restore in their own `finally` blocks.
 */
