...
The current workspace is exposed live on the `BridgeService`:
...
@Component({
  ...
  standalone: true,
  // Each field is its own reactive signal; call it to read the current value.
...
A drop-in `<bridge-workspace-selector>` component lists the workspaces the signed-in user can access and switches the active one; the same enabled-and-active filter from workspace selection at login applies here too. On switch, the SDK issues a fresh session for the chosen workspace and refreshes the whole `BridgeService`, including that person's role, which may not be the same in the new workspace as it was in the last one.

Inputs, usage, and styling hooks live on the component's own page: [Switching workspaces](/auth/ui/switching-workspaces/).
