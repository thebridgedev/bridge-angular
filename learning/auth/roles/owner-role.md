...
try {
  await authService.getBridgeAuth().team.updateUser({ id: userId, role: 'MEMBER' });
} catch (err) {
...
`<bridge-team-panel>` (see [User & Team management](/auth/ui/team-management/)) already handles this for you: the error surfaces as a normal form error in the drop-in UI, so you only need the snippet above if you're building your own role-management screen instead.
