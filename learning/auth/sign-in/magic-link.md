...
- **Keep the requesting route reachable**, and render either `<bridge-magic-link>` or `<bridge-login-form>` on it. Both read `bridge_magic_link_token` on init, exchange it for a session and strip it from the URL — there's nothing else to wire up.
...
export class MagicLinkRequestComponent {
  ...
  async requestLink(email: string): Promise<void> {
    await this.authService.getBridgeAuth().sendMagicLink(email, {
...
That page needs to redeem the token too, so it should also render `<bridge-magic-link>` or `<bridge-login-form>`.
