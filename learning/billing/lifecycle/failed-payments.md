...
(automated payment retries). All of this arrives live on the `bridge` surface;
you don't poll Stripe.

## Surfacing it to users

`<bridge-billing-notice />` is built for this: it renders a past-due / payment-problem
...
workspace can re-subscribe, or use `<bridge-billing-notice mode="hard" />` for a
