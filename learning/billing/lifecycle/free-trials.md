...
trial end date, both live on the `bridge` surface (see
[How billing works](/billing/how-it-works/)).

## Surfacing trial state

The drop-in components already understand trials: `<bridge-subscription-status />`
shows the trialing status and `<bridge-billing-notice />` surfaces an
