...
auth.getBridgeAuth().usage.report('ai_completions');

// Report N units at once
auth.getBridgeAuth().usage.report('tokens', 1375);
```
...
```ts
auth.getBridgeAuth().usage.report('report_generated', 1, `report:${reportId}`);
```
...
```ts
const status = await auth.getBridgeAuth().usage.getQueueStatus();
