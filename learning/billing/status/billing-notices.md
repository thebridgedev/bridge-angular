...
@Component({
  ...
  // Put it once in your root component's template
...
> **Framework note:** The default CTA navigates to `billing.manageRoute` from
> your Bridge config, falling back to `/billing`. If your plan page lives at
> `/subscription`, set `billing: { manageRoute: '/subscription' }` in
> `provideBridge()`; use `actionHref` for a one-off destination or
> `onActionClick` to take over the click entirely (TBP-451). Role detection is
> not wired yet: any authenticated user currently sees the admin variant.
