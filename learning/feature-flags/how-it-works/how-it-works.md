...
- **No network on read.** `bridge.flag` / `<bridge-feature-flag>` evaluate against an
...
You can observe the connection yourself: `realtimeStatus` is an Angular
signal of the current `ConnectionState`, which is one of `'idle'`,
`'connecting'`, `'open'`, or `'closed'`. Read it in a component template by
calling it:
...
export class OfflineBannerComponent {
  protected readonly status = realtimeStatus;
}
```

> **Framework note:** the same signal is also exposed as
> `BridgeService.realtimeStatus` if you already inject the service.
