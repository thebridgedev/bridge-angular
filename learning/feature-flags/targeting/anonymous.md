...
Most apps never touch this; the default `provideBridge()` wiring uses
persistent tracking.
...
@Component({
  ...
  standalone: true,
  imports: [NewLandingComponent, OldLandingComponent],
  template: `
    @if (landing().value) {
      <app-new-landing />
    } @else {
...
export class LandingComponent {
  ...
  protected readonly landing = this.bridge.flag('new_landing', false);
}
```
