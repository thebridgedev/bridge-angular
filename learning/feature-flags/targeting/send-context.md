...
> **Framework note:** `bridge.flag` captures its per-call context when the
> accessor is created (in a field initializer or constructor). For an attribute
> whose value changes over time, use `bridge.attributes.bind` (below) or the
> `<bridge-feature-flag>` component, whose `[context]` input re-evaluates on
> change.

`<bridge-feature-flag>` takes the same context as a `[context]` input instead of a third argument, and reactivity works slightly differently between the two; see [Show or hide UI](/feature-flags/using/show-hide-ui/#sending-context) for the component form and the difference.
