# Bridge Angular — Branding

You are applying a workspace's branding (logo, name, colours) inside an Angular application that uses The Bridge.
...
`bridge.app.branding` is an Angular `Signal`, so it works directly in a template:
...
@Component({
  ...
  template: `
    @if (branding(); as b) {
      <header [style.background]="b.bgColor ?? 'var(--surface)'"
              [style.color]="b.textColor ?? 'inherit'">
...
export class HeaderComponent {
  readonly branding = inject(BridgeService).app.branding;
}
```

The `@if (branding(); as b)` guard matters: the signal is `null` until the first snapshot arrives.
