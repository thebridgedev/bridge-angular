#!/usr/bin/env bash
#
# Static sweep for `networkidle` waits in the Playwright suite. (Origin: bridge-svelte TBP-605; ported to Angular in TBP-721.)
#
# Why it matters: the demo holds a persistent realtime WebSocket, so the
...
# one frame away. In bridge-svelte eight subscription specs failed this way, all parked
...
if [ -n "$hits" ]; then
  ...
  printf '%s\n' "$hits" | sed 's/^/    /' >&2
  echo "" >&2
  echo "  The demo keeps a realtime WebSocket open, so the network never goes" >&2
